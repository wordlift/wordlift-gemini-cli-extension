...
def generate_entity_id(dataset_uri: str, entity_class: str, natural_key: str) -> str:
    ...
    ⚠️ CRITICAL: WordLift requires specific path patterns. Using arbitrary paths
    will cause silent failures (API returns 200 OK but entity NOT persisted).

    Format: {dataset_uri}/{entity_class}/{slug}

    Valid entity_class values:
    - organization, place, person, destination, article, webpage, brand, event, service

    Args:
        dataset_uri: Base dataset URI (HTTPS, no trailing slash)
        entity_class: Entity class name (must be from valid list above)
    ...
    Raises:
        ValueError: If entity_class is not a recognized WordLift pattern
    ...
    # Validate entity_class against known WordLift patterns
    valid_patterns = [
        'organization', 'place', 'person', 'destination',
        'article', 'webpage', 'brand', 'event', 'service'
    ]

    if entity_class not in valid_patterns:
        raise ValueError(
            f"Entity class '{entity_class}' may not be persisted. "
            f"WordLift requires specific path patterns. "
